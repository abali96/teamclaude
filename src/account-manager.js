...
export class AccountManager {
   ...
   * priority → preserve Fable quota → [top pressure band, when expiry
   * routing is on] → fewest active sessions → fewest in-flight → highest
   * expiry pressure (inert when expiry routing is off) → soonest weekly reset
  ...
  _pickLeastLoadedEven(exclude = null, model = null, advisorModel = null) {
    ...
    candidates.forEach((account, i) => {
      ...
      const sameFable = samePriority && fableRank === bestFableRank;
      const sameSpend = sameFable && heldOff === bestSpent;
      if (priority < bestPriority
        || (samePriority && fableRank < bestFableRank)
        || (sameFable && heldOff < bestSpent)
  ...
  previewRouteIndex(model, provider = DEFAULT_PROVIDER) {
    ...
    if (current && this._isAvailable(current, model)) {
      ...
      const better = this._preemptedBy(current, model, null, excluded);
      const rolled = this.expiryRouting.enabled && this.expiryRouting.preempt
        && this._currentRolledOver(current, model);
   ...
   *      discover its quota
   *   4. then the account with the most expiring quota (headroom per second
   *      until its window resets), when expiry routing is on
   *   5. then the account whose weekly limit expires soonest: that quota is
  ...
  _pickBestAvailable(exclude = null, model = null, advisorModel = null) {
    ...
    candidates.forEach((account, i) => {
      ...
      if (priority < bestPriority
          ...
          || (priority === bestPriority && fableRank === bestFableRank && pressure < bestPressure)
          || (priority === bestPriority && fableRank === bestFableRank && pressure === bestPressure && weeklyReset < bestReset)) {
        bestPriority = priority;
        bestFableRank = fableRank;
        bestPressure = pressure;
