...
The menu-bar label finds the account with the lowest known Fable usage and shows that same account's Fable, session, and weekly usage with each bucket's reset (`Fable: 40% (3d 8h) · Session: 90% (2h 18m) · Week: 10% (6d 2h)`). Resets under 24 hours show hours and minutes in both the menu-bar label and the account rows; longer resets show days and hours. Open it to see every account's session, weekly, Sonnet, and Fable usage, account state, and active session count; the account supplying the summary is marked `lowest Fable`. Clicking an account calls the same runtime switch endpoint as `teamclaude switch`; it does not rewrite the config. The menu refreshes every ten seconds, refreshes immediately when opened, and clearly shows when the proxy is disconnected.
