...
| `fableSpentThreshold` | Fable weekly utilization (0–1, default `0.9`) at which an account counts as **out of Fable** for the preservation rule: non-Fable requests prefer such equal-priority accounts, keeping the shared 5h window on Fable-capable accounts free for Fable. Independent of `switchThreshold`; if the Fable rotation threshold is lower, that governs. Applied live on reload |
